...
def import_ami(s):
    ...
    while 'UnencryptedSnapshotId' not in s:
        res = ec2.describe_import_snapshot_tasks(
            ImportTaskIds = [s['ImportTaskId']],
        )
        ...
        if detail['Status'] == 'active':
            ...
            raise Exception(f'''Unknown import snapshot task status: {detail['Status']}''')
    ...
    while True:
        res = ec2.describe_snapshots(
            SnapshotIds = [s['EncryptedSnapshotId']],
        )
    ...
    res = ec2.delete_snapshot(
        SnapshotId = s['UnencryptedSnapshotId'],
    )
    ...
    res = ec2.register_image(
        ...
        BlockDeviceMappings = [{
            'DeviceName': '/dev/xvda',
            'Ebs': {
                'SnapshotId': s['EncryptedSnapshotId'],
                'VolumeType': 'gp2',
            }
        }],
        EnaSupport = True,
